...
class PopUp():
    ...
    def SetUpUI(self, Header, Link, TextToCopy, CanCopy) -> None:
        ...
        if (CanCopy):
            self.CopyButton = tk.Button(master=self.window, text="Copy", command=lambda: [pyperclip.copy(TextToCopy)], bg="#92DDC8", foreground="#121212", borderwidth=0)
        ...
        self.LinkButton = tk.Button(master=self.window, text=LinkButtonText, command=lambda: [self.OpenLink(Link)], bg="#92DDC8", foreground="#121212", borderwidth=0)
